...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    mkdir -p data/train
    mkdir -p data/${ta_test_set}

    python local/msr_data_prep.py --source ${ta_datadir}/ta-in-Train --destination data/train
    python local/msr_data_prep.py --source ${ta_datadir}/ta-in-Test --destination data/${ta_test_set}
    ...
    utils/subset_data_dir_tr_cv.sh --cv-spk-percent 20 data/train data/ta_train data/ta_dev
    # local/split_tr_dt_et.sh data/train data/ta_train data/ta_dev data/ta_test_2
    ...
    # filter out the same sentences (also same text) of test&dev set from validated set
     ...
     for part in ${ta_train_set} ${ta_dev_set} ${ta_test_set}; do
    ...
    # utils/subset_data_dir.sh --speakers data/libri_tra/ 75000 data/cv_valid_train_fr_subset
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # utils/subset_data_dir.sh --speakers data/libri_train_clean_100 1000  data/libri_train_subset 
    for part in dev-clean test-clean train-clean-100; do
    ...
    utils/subset_data_dir.sh --speakers data/libri_train_clean_100 10000 data/libri_train_subset
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    if [ $exp_lang = "both" ] 
    then
        utils/data/combine_data.sh data/train_nodev data/libri_train_subset/ data/ta_train/
        utils/data/combine_data.sh data/train_dev data/libri_dev_clean/ data/ta_dev/
	recog_set="${libri_recog_set} ${ta_test_set}"
    elif [ $exp_lang = "en" ] 
    then
        rm -rf data/train_nodev data/train_dev
        utils/copy_data_dir.sh data/libri_train_subset/ data/train_nodev
        utils/copy_data_dir.sh data/libri_dev_clean/ data/train_dev
	recog_set="${libri_recog_set}"
    elif [ $exp_lang = "ta" ] 
    then
        rm -rf data/train_nodev data/train_dev
        utils/copy_data_dir.sh data/ta_train/ data/train_nodev
        utils/copy_data_dir.sh data/ta_dev/ data/train_dev
	recog_set="${ta_test_set}"
    else
        echo "Wrong choice for exp_lang. It should be one of [both, en, ta]. You have given $exp_lang"
        exit 0
    fi
fi
